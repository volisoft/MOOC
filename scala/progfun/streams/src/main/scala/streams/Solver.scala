...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        (recentBlock, recentHistory) <- initial
        possibleMoves = neighborsWithHistory(recentBlock, recentHistory)
        move <- newNeighborsOnly(possibleMoves, explored)
      } yield move
//      val (recentBlock, recentHistory) = initial.head
//      val possibleMoves = neighborsWithHistory(recentBlock, recentHistory)
//      val more: Stream[(Block, List[Move])] = newNeighborsOnly(possibleMoves, explored)
