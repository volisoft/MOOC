...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val x = StructField(head, StringType, nullable = false)
    val xs = tail.map(StructField(_, DoubleType, nullable = false))
    StructType(x :: xs)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def inGroup(x: String, group: List[String]): Boolean = group.exists(x.startsWith)

    def toCols(names: List[String], filter: String => Boolean): List[Column] = for (name <- names if filter(name)) yield new Column(name)

    def isPrimary(x: String): Boolean = inGroup(x, primaryNeeds)
    def isWorking(x: String): Boolean = inGroup(x, workingActivities)
    def isOther(x: String): Boolean = inGroup(x, otherActivities) && !isPrimary(x) && !isWorking(x)

    val primary = toCols(columnNames, isPrimary)
    val working = toCols(columnNames, isWorking)
    val other= toCols(columnNames, isOther)
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"SELECT working, sex, age, AVG(primaryNeeds) as primaryNeeds, AVG(work) as work, AVG(other) as other from $viewName " +
